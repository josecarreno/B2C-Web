package pe.com.b2c.ws.constants;

public class InmuebleURIConstants {
    
	public static final String GET_INMUEBLE = "/inmueble/{idInmueble}";
	public static final String GET_ALL_INMUEBLE = "/inmuebles";
	public static final String BUSCAR_INMUEBLES = "/inmuebles/buscar";
        public static final String INMUEBLES_PROPIOS = "/inmuebles/propios/{idUsuario}";
	public static final String CREATE_INMUEBLE = "/inmueble/create";
        public static final String UPDATE_INMUEBLE = "/inmueble/update";
	public static final String DELETE_INMUEBLE = "/inmueble/delete/{idInmueble}";
        
}
